...
- 日志与客户端协议范围检查已通过：客户端保留现有 `socket_addr_v6` 单字段，Flutter UI 无 diff。
- 2026-07-01 Mac -> Windows runtime evidence 显示旧 hbbs 未转发 `socket_addr_v6`，因此 server deploy 是 E2E 前置条件。
- 多设备 E2E 未完成：Windows 端必须运行同 commit fork artifact，且 ID server 必须运行 patched hbbs 后，再验证最终连接类型、UDP/IPv6 direct 和延迟 baseline。
...
| REQ-3.3 | `design.md#Existing Rendezvous Path`, `design.md#Logging` | IMPL-003, IMPL-004 | final connection type log path present；2026-07-01 实测为 Relay，blocked by old hbbs | blocked-server |
...
| REQ-6.3 | `design.md#Forbidden Patterns` | IMPL-003 | client protobuf 无 diff；server protobuf 需 patched hbbs 转发 `socket_addr_v6` | updated-runtime |
...
| SERVER-1 | `development-pipeline.md#部署-patched-hbbs-前置条件` | server patch | `cargo check --locked` in `/Users/neko/Documents/Project/rustdesk-server` | passed-code |
| SERVER-2 | `development-pipeline.md#部署-patched-hbbs-前置条件` | server deploy | actual ID server runs patched `hbbs` and logs `socket_addr_v6_non_empty=true` | pending-deploy |
...
cd /Users/neko/Documents/Project/rustdesk-server && cargo check --locked
cd /Users/neko/Documents/Project/rustdesk-server && cargo build --release --locked --bins
```

## 未关闭项

- Windows 同 commit artifact 运行。
- patched hbbs 部署到实际 ID server。
